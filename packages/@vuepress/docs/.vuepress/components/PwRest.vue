	...
	<section class='pw-rest'>
		...
		<main class="area request">
			...
			<div class="pw-rest__item example">
				<header>
					<h4 class="label">Request example</h4>
					<div>
				...
				<div 
					...
					><pre><code>{{ ui.requestExample }}</code></pre><span class="onhover">Click to copy</span></div>
			</div>
...
xhr.send()`
		...
		send (fullUrl, method) {
			...
			this.api.message = 'Sending request...'
			...
			oReq.addEventListener("abort", evt => {
				this.api.fetching = false
				this.api.message = 'Error! The request was canceled'
			})
...
.pw-rest {
	...
	padding: 4.5rem 4.5rem 2.5rem 2.5rem;
